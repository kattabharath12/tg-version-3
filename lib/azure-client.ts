...
export class AzureDocumentClient {
  ...
  // Enhanced method that returns human-readable field values
  extractFieldsFromResult(result: AzureDocumentAnalysisResult): Array<{
    ...
    for (const document of result.documents) {
      for (const field of document.fields) {
        // Extract human-readable value from complex field structure
        const readableValue = this.extractReadableValue(field.fieldValue);
    ...
    console.log(`📊 Extracted ${fields.length} fields from ${result.documents.length} documents`);
    if (fields.length > 0) {
      console.log(`📝 Sample fields: ${fields.slice(0, 3).map(f => `${f.fieldName}=${f.fieldValue}`).join(', ')}`);
    }
  ...
  /**
   * Extract human-readable string value from complex Azure field structure
   * This method handles nested objects, arrays, and simple values
   */
  private extractReadableValue(fieldData: any): string | null {
    ...
    try {
      ...
      if (fieldData.value !== undefined && fieldData.value !== null) {
        ...
        if (typeof fieldData.value === 'string' || typeof fieldData.value === 'number' || typeof fieldData.value === 'boolean') {
          return String(fieldData.value);
        }
        
        // Check for nested valueNumber or valueString
        if (fieldData.value.valueNumber !== undefined) {
          return String(fieldData.value.valueNumber);
        }
        if (fieldData.value.valueString !== undefined) {
          return String(fieldData.value.valueString);
        }
        if (fieldData.value.valueDate !== undefined) {
          return String(fieldData.value.valueDate);
        }
      }

      // Handle array type (like Transactions)
      if (fieldData.type === 'array' && Array.isArray(fieldData.value)) {
        const extractedValues: string[] = [];
        
        fieldData.value.forEach((item: any, index: number) => {
          if (item.type === 'object' && item.value) {
            // Extract all box values from the transaction
            Object.entries(item.value).forEach(([boxName, boxValue]: [string, any]) => {
              const boxVal = this.extractReadableValue(boxValue);
              if (boxVal) {
                extractedValues.push(`${boxName}:${boxVal}`);
              }
            });
          }
        });
      ...
      if (fieldData.type === 'object' && fieldData.value) {
        const extractedValues: string[] = [];
        Object.entries(fieldData.value).forEach(([key, value]: [string, any]) => {
          const val = this.extractReadableValue(value);
          if (val) {
            extractedValues.push(`${key}:${val}`);
          }
        });
      ...
      // Handle address type
      if (fieldData.type === 'address' && fieldData.value) {
        const addressParts: string[] = [];
        if (fieldData.value.streetAddress) addressParts.push(fieldData.value.streetAddress);
        if (fieldData.value.city) addressParts.push(fieldData.value.city);
        if (fieldData.value.state) addressParts.push(fieldData.value.state);
        if (fieldData.value.postalCode) addressParts.push(fieldData.value.postalCode);
      ...
      // Fallback: Try to stringify in a compact way
      if (typeof fieldData === 'object') {
        // For small objects, show compact JSON
        const str = JSON.stringify(fieldData);
        if (str.length < 200) {
          return str;
        }
        // For large objects, just indicate it's complex data
        return '[Complex Object Data]';
      }

      return String(fieldData);
      
    } catch (error) {
      console.warn('Failed to extract readable value from field:', error);
      // Return compact JSON as fallback
